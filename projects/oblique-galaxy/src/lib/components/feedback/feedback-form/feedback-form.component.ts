...
export class FeedbackFormComponent {
	...
	constructor() {
		this.controls.forEach(control => {
			this.formGroup.addControl(control.name, new FormControl<string>(control.defaultValue ?? '', {validators: control.validators, nonNullable: true}));
			//@ts-expect-error @typescript-eslint/ban-ts-comment
	...
	private validateMaxLength(errors: RequiredLength): string {
		...
		return `Please enter at most ${errors.requiredLength} characters. You have ${overflowing} character${this.pluralize(overflowing)} too much.`;
	}
